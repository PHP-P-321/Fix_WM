...
<body>
    ...
    <?php if($_COOKIE['role'] == 1) { ?>
        ...
        <div class="wrapper">
            <div>
                <h2>Стиральные машины в ремонте</h2>
                <?php 
                    foreach($select_tv_in_repair as $tv) { ?>
                        <ul>
                            <li><strong>Название производителя: </strong> <?= $tv[3] ?> </li>
                            <li><strong>Название модели машины: </strong> <?= $tv[4] ?> </li>
                            <li><strong>Описание проблемы: </strong> <?= $tv[5] ?> </li>
                        </ul>
            ...
            <div>
                <h2>Стиральные машины на утверждение ремонта</h2>
                <?php 
                    foreach($select_tv_before_repair as $tv) { ?>
                        <ul>
                            <li><strong>Название производителя: </strong> <?= $tv[3] ?> </li>
                            <li><strong>Название модели телевизора: </strong> <?= $tv[4] ?> </li>
                            <li><strong>Описание проблемы: </strong> <?= $tv[5] ?> </li>
    ...
    <?php } elseif ($_COOKIE['role'] == 2) { ?>
        ...
        <form action="./vendor/create-request.php" method="post">
            ...
            <textarea name="type_of_fault" id="" cols="30" rows="10" placeholder="Описание проблемы" required></textarea>
